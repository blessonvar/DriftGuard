...
from driftguard.federate.server.fed_server import FedServerArgs
from driftguard.federate.client.client import FedClientArgs
from driftguard.data.service import DataServiceArgs
...
from driftguard.runtime.interfaces import DataServiceEndpoint, ServerEndpoint
from driftguard.runtime.ray.actors import (
    RayDataServiceActor,
...
_retraining_thresholds = _config["retraining_thresholds"]
_global_retraining_thresholds = _config["global_retraining_thresholds"]
_clustering_distance = _config["clustering_distance"]
_min_cluster_size = _config["min_cluster_size"]
_strategies_map = {
    "Never": Never(),
    "FCL-AveTrig": AveTrig(thr_acc=_retraining_thresholds),
    "FCL-perDevice": PerCTrig(thr_acc=_retraining_thresholds),
    "PFL-AveTrig": MoEAve(thr_acc=_retraining_thresholds),
    "PFL-perDevice": MoEPerC(thr_acc=_retraining_thresholds),
    "Cluster-based ": Cluster(thr_acc=_retraining_thresholds),
    "DriftGuard": Driftguard(
        thr_group_acc=_global_retraining_thresholds,
        thr_sha_acc_pct=_global_retraining_thresholds,
        cluster_thr=_clustering_distance,
        min_group_size=_min_cluster_size,
    ),
}
_strategies = [_strategies_map[name] for name in _config["strategies"]]
...
def main() -> None:
    ...
    for exp in exps:
        ...
        data_args = DataServiceArgs(
            meta_path= Path.cwd() / cfg.dataset.path,
        ...
        data_actor = RayDataServiceActor.options(
            resources={"server": 0.01 if _pi else 0},
        ).remote(data_args)
            ...
            client_actors = [
                *[
                    RayFedClientActor.options(
                        ...
                        resources={"client": 1 if _pi else 0},
                    ).remote(
                        build_client_args(
                            cid,
                            cfg,
                            ...
                            server_ep,
                            resource={
                                "num_gpus": _config.get("gpu_per_client", 0) if "cuda" in cfg.device else 0,
                                "training_node": 1 if _pi else 0,
                            },  # _train need
                        )
                    )
